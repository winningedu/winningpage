...
describe("buildHeroNarrative — 기록 없음(QA 2차 시트 행60)", () => {
  ...
  it("지난달 기간이고 기록이 없으면 '지난달' 문구로 안내한다", () => {
    const text = buildHeroNarrative({
      period: "monthly",
      totalStudyHours: 0,
      idealRate: 0,
    ...
    expect(text).toBe(
      "지난달(2026.8.1~2026.8.31) 학습목표 또는 실행내역의 입력 내용이 없습니다. 학습목표의 관리를 위해서는 지속적인 목표관리 체크 및 현황 입력이 중요합니다.",
    );
  });

  it("진행 중인 이번 달 기간이고 기록이 없으면 '이번 달' 문구로 안내한다", () => {
    const text = buildHeroNarrative({
      period: "monthly",
      totalStudyHours: 0,
      idealRate: 0,
    ...
    expect(text).toBe(
      "이번 달(2026.9.1~2026.9.30) 학습목표 또는 실행내역의 입력 내용이 없습니다. 학습목표의 관리를 위해서는 지속적인 목표관리 체크 및 현황 입력이 중요합니다.",
    );
  });
});
